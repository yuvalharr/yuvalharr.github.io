...
jsPsych.plugins["YH-rdm"] = (function() {
  ...
  plugin.info = {
    ...
    parameters: {  // Define all input parameters and their corresponding default values
      ...
      color: {
        ...
        default: 0, // 0 = yellow | 1 = blue | 2 = white
      },
      color_rgb: {
        type: jsPsych.plugins.parameterType.INT, 
        array: true,
        pretty_name: "Dot color",
        default: [[232,232,0], // yellow
                [0,232,232], // blue
                [255,255,255]], // white
      },
      response_keys: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    var RDM = {
      ...
      color: trial.color,
      color_coh: trial.color_coherence/100,
    ...
    if (trial.decision_task == '1D_motion') {
      //trial.response_keys = trial.response_keys_motion;
      data.correctChoice = trial.motion_direction;
    } else if (trial.decision_task == '1D_color') {
      //trial.response_keys = trial.response_keys_color;
      data.correctChoice = trial.color;
    }
      ...
      function stateProcess() {
        ...
        if (trial.demo_trial) {
          ...
          if ((State.Current === State.RESPONSE_WAIT && State.Time > 750) || (State.Current === State.FEEDBACK )) {
            if (trial.decision_task == '1D_motion' && trial.motion_direction === 0) {
              instructionText = 'In this example, the majority of dots move LEFT\n[Press '+trial.response_keys[0].toUpperCase() +']';
            } else if (trial.decision_task == '1D_motion' && trial.motion_direction === 1) {
              instructionText = 'In this example, the majority of dots move RIGHT\n[Press '+trial.response_keys[1].toUpperCase() +']';
            } else if (trial.decision_task == '1D_color' && trial.color === 0) {
              instructionText = 'In this example, the majority of dots is YELLOW\n[Press '+trial.response_keys[0].toUpperCase() +']';
            } else if (trial.decision_task == '1D_color' && trial.color === 1) {
              instructionText = 'In this example, the majority of dots is BLUE\n[Press '+trial.response_keys[1].toUpperCase() +']';
            }
          }
        ...
        switch (State.Current) {
          ...
          if (State.Current<=State.FINISH) {
            ...
            if (trial.decision_task === '1D_motion') {
              ...
              draw_text(trial.response_keys[0].toUpperCase()+'\nYellow',[centerX-0.25*centerX,centerY+0.7*centerY],16,'rgb(' + trial.color_rgb[0] + ')',true);
              draw_text(trial.response_keys[1].toUpperCase()+'\nBLUE',[centerX+0.25*centerX,centerY+0.7*centerY],16,'rgb(' + trial.color_rgb[1] + ')',true);
            }
            ...
            if (State.Current >= State.FIXATE) {
              ...
              if ((State.Current==State.RDM_ON || State.Current==State.RESPONSE_WAIT) && frameID < RDM.X.length-1) {
                for (i=0; i<RDM.dots_per_channel; i++) {
                  draw_circle(centerX+RDM.X[frameID][i],centerY+RDM.Y[frameID][i],RDM.size,'rgb(' + trial.color_rgb[RDM.dot_col[frameID][i]] + ')',1,1);
                }
    ...
    function end_trial() {
      ...
      var trial_data = { 
        "Task": trial.decision_task, // decision task (1D motion, 1D color, 2D)
        "MotCoh": trial.motion_coherence, // motion coherence
        "MotDir": trial.motion_direction, // motion direction
        "ColCoh": trial.color_coherence, // color coherence
        "Color": trial.color, // dominant color
