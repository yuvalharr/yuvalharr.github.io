    ...
    var keyboardListener = jsPsych.pluginAPI.getKeyboardResponse({
      callback_function: function(info){
          ...
          if (response.rt > trial.hideButtonDuration) {
            data.RT = response.rt;
            data.choice_key = response.key; //data.choice_key = jsPsych.pluginAPI.convertKeyCodeToKeyCharacter(response.key);
            choice = trial.choice_keys; //choice = trial.choice_keys.findIndex(x => x === data.choice_key.toUpperCase());
            if (choice === -1) {
              choice = 0;
            }
            //console.log(choice)
