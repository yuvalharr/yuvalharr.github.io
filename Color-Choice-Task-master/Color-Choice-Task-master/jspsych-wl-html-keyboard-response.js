    ...
    var keyboardListener = jsPsych.pluginAPI.getKeyboardResponse({
      callback_function: function(info){
          ...
          if (response.rt > trial.hideButtonDuration) {
            data.RT = response.rt;
            data.choice_key = jsPsych.pluginAPI.convertKeyCodeToKeyCharacter(response.key); // original
            choice = trial.choice_keys.findIndex(x => x === data.choice_key.toUpperCase()); // original
            if (choice === -1) {
              choice = 0;
            }
            //console.log(choice)
