...
all_summary <- merge(all_summary, both_sessions) # add both sessions data to all_summary
...
cor_matrix <- all_summary[,c(8,14,16,19:20,4,6,2,12,23:24,9)] # for presentation only


setnames(cor_matrix, 'cb_acc', "cb_acc_60s")
setnames(cor_matrix, 'brms_rt', "NPS 1")
setnames(cor_matrix, 'brms_rt_2', "NPS 2")
setnames(cor_matrix, 'both_brms', "NPS Combined")
setnames(cor_matrix, 'control_rt_1', "Control RT 1")
setnames(cor_matrix, 'control_rt_2', "Control RT 2")
setnames(cor_matrix, 'cb_rt_with_unfinished', "CB RT")
setnames(cor_matrix, 'view_dist_mm', "Viewing Distance 1")
...
a<- partial.r(cor_matrix, x = c("NPS Combined", "cb_acc_20s","cb_acc_40s", "cb_acc_60s","CB RT", "alpha"), y = c("Control RT 1", "Control RT 2"), use="pairwise",method="pearson")
...
temp <- cor_matrix[complete.cases(cor_matrix[ , c('NPS Combined', 'Control RT 1', 'cb_acc_20s', 'cb_acc_40s', 'cb_acc_60s', 'CB RT', 'alpha')]),]
b <- pcor(temp[,c('NPS Combined', 'Control RT 1', 'cb_acc_20s', 'cb_acc_40s', 'cb_acc_60s', 'CB RT', 'alpha')])

######################################


# Make a table only for brms rt
p_for_correction <- cor_results$p["NPS 1",]
r_for_correction <- cor_results$r["NPS 1",]
...
temp <- all_summary[complete.cases(all_summary[ , c('brms_rt', 'cb_rt', 'control_rt_1')]),]
pcor.test(x=temp[,brms_rt], y=temp[, cb_rt], z=temp[, control_rt_1]) # account for control
...
cor_matrix <- all_summary[brms_rt < 5000,c(2,4,6,8:9,12,14,19,23:24)]
setnames(cor_matrix, 'cb_acc', "cb_acc_60s")
...
cor_matrix <- all_summary[brms_rt < 10000,c(2,4,6,8:9,12,14,19,23:24)]
setnames(cor_matrix, 'cb_acc', "cb_acc_60s")
setnames(cor_matrix, 'cb_rt_with_unfinished', "cb_rt")
...
cor_matrix <- all_summary[brms_rt < 20000,c(2,4,6,8:9,12,14,19,23:24)]
setnames(cor_matrix, 'cb_acc', "cb_acc_60s")
setnames(cor_matrix, 'cb_rt_with_unfinished', "cb_rt")
cor_results <- corr.test(cor_matrix, adjust = "hoch")
...
all_summary <- data.table(all_summary)
cor.test(all_summary[brms_rt < 20000, brms_rt], all_summary[brms_rt < 20000, alpha])
ggplot(all_summary[brms_rt < 20000], aes(x = brms_rt, y = alpha)) +
  geom_point() +
  geom_smooth(method='lm') 
pcor.test(x=all_summary[brms_rt < 20000, brms_rt], y=all_summary[brms_rt < 20000, alpha], z=all_summary[brms_rt < 20000, distance]) # account for distance

model <- lm(brms_rt ~ alpha + cb_acc_20s + view_dist_mm, data = all_summary)
summary(model)
...
pcor.test(x=all_summary$cb_rt_with_unfinished, y=all_summary$brms_rt, z=all_summary$view_dist_mm)
...
cor.test(all_summary$brms_rt, all_summary$driving_ability)
ggplot(all_summary, aes(x = brms_rt, y = driving_ability)) +
  geom_point() +
  geom_smooth(method='lm') +
  ggtitle("Correlation between NPS and driving ability")+
  stat_cor(method="pearson", color = 'red', label.y.npc="top", label.x.npc = "center" )

cor.test(all_summary$brms_rt, all_summary$driving_accidents)
ggplot(all_summary, aes(x = brms_rt, y = driving_accidents)) +
  geom_point() +
  geom_smooth(method='lm') +
...
cor.test(only_cocky_drivers$brms_rt, only_cocky_drivers$driving_ability)
ggplot(only_cocky_drivers, aes(x = brms_rt, y = driving_ability)) +
  geom_point() +
  geom_smooth(method='lm') +
  ggtitle("Correlation between NPS and driving ability")+
  stat_cor(method="pearson", color = 'red', label.y.npc="top", label.x.npc = "center" )

cor.test(only_cocky_drivers$brms_rt, only_cocky_drivers$driving_accidents)
ggplot(only_cocky_drivers, aes(x = brms_rt, y = driving_accidents)) +
  geom_point() +
  geom_smooth(method='lm') +
  ggtitle("Correlation between NPS and driving accidents")+
  stat_cor(method="pearson", color = 'red', label.y.npc="top", label.x.npc = "center" )
pcor.test(x=only_cocky_drivers$brms_rt, y=only_cocky_drivers$driving_accidents, z=only_cocky_drivers$age) # account for distance
...
cor.test(only_cocky_drivers$brms_rt, only_cocky_drivers$driving_ability)
ggplot(all_summary[brms_rt <5000,], aes(x = brms_rt, y = beta)) +
  geom_point() +
  geom_smooth(method='lm') +
  ggtitle("")+
  stat_cor(method="pearson", color = 'red', label.y.npc="top", label.x.npc = "center" )
