...
#  ungroup()
#refreshed <- data.table(refreshed)
...
finished_exp <- unique(dt[cb_trial == 30, id]) # mark as finished if last cb trial exists
unfinished <- dt[!(id %in% finished_exp)] # unfinished experiments are saved here before removal
dt <- dt[id %in% finished_exp,] # keep only finished experiments
...
trialCount <- trialCount[trials >= 30]
only_brms <- only_brms[id %in% trialCount$id]
#only_animation <- only_animation[id %in% trialCount$id] # removed for now. Now taking all CB even if brms data is from both sessions together or the 2nd alone
...
distance_dt <- dt[!(is.na(view_dist_mm)),.(id, view_dist_mm)]

# merge distance with brms summary
brms_summary <- only_brms[, .(brms_rt = mean(rt)), by = id]
brms_summary <- merge(brms_summary,distance_dt, by = 'id')

ggplot(brms_summary, aes(x = brms_rt, y = view_dist_mm)) +
  geom_point() +
...
all_summary <- merge(brms_summary, cb_summary, by = 'id', all = T)
all_summary <- merge(all_summary, both_sessions, all = T) # add both sessions data to all_summary
only_animation <- merge(only_animation, all_summary[,.(brms_rt_both, id)]) # add mean brms score for each sbj
...
p <- ggplot(brms_summary, aes(x=as.factor(1), y=brms_rt_both)) + 
  geom_violin()
...
h = hist(all_summary$brms_rt_both, breaks = c(seq(from = 0, to = 30000, by = 1000)), plot = F) # or hist(x,plot=FALSE) to avoid the plot of the histogram
h$density = h$counts/sum(h$counts)*100
plot(h,freq=FALSE, main = "NPS distribution - session 1", xlab = "NPS (ms)", ylab = "% of participants")
abline(v = mean(all_summary$brms_rt_both, na.rm = T), col = 'red')

# histogram of average BRMS- session 2
h = hist(all_summary$brms_rt_2, breaks = c(seq(from = 0, to = 30000, by = 1000)), plot = F) # or hist(x,plot=FALSE) to avoid the plot of the histogram
h$density = h$counts/sum(h$counts)*100
plot(h,freq=FALSE, main = "NPS distribution - session 2", xlab = "NPS (ms)", ylab = "% of participants")
abline(v = mean(all_summary$brms_rt, na.rm = T), col = 'red')

# histogram of average Control brms
h = hist(all_summary$control_rt_1, breaks = c(seq(from = 0, to = 30000, by = 1000)), plot = F) # or hist(x,plot=FALSE) to avoid the plot of the histogram
h$density = h$counts/sum(h$counts)*100
plot(h,freq=FALSE, main = "Control 1 distribution", xlab = "NPS (ms)", ylab = "% of participants")
abline(v = mean(all_summary$control_rt_1, na.rm = T), col = 'red')

h = hist(all_summary$control_rt_2, breaks = c(seq(from = 0, to = 30000, by = 1000)), plot = F) # or hist(x,plot=FALSE) to avoid the plot of the histogram
h$density = h$counts/sum(h$counts)*100
plot(h,freq=FALSE, main = "Control 2 distribution", xlab = "NPS (ms)", ylab = "% of participants")
abline(v = mean(all_summary$control_rt_2, na.rm = T), col = 'red')
...
h = hist(cb_summary$cb_rt_with_unfinished/1000, breaks = c(seq(from = 0, to = 40, by = 2.5)), plot = F) # or hist(x,plot=FALSE) to avoid the plot of the histogram
h$density = h$counts/sum(h$counts)*100
...
# Make BRMS quentiles
library(dplyr)
all_summary$brms_quentile <- ntile(all_summary$brms_rt_both, 10)  
...
only_animation <- only_animation[, brms_rt_both:=as.double(brms_rt_both)] # make rt column as double instead of string

mid <- mean(all_summary$brms_rt_both)
d.f <- arrange(only_animation,brms_rt_both,rt)

d.f.ecdf <- ddply(d.f, .(brms_rt_both), transform, ecdf=ecdf(rt)(rt) )
p <- ggplot( d.f.ecdf, aes(rt, ecdf, group = id, colour = brms_rt_both))
...
d.f_10s <- arrange(only_animation,brms_rt_both,rt)
...
d.f_20s <- arrange(only_animation,brms_rt_both,rt)
...
d.f_30s <- arrange(only_animation,brms_rt_both,rt)
...
d.f_40s <- arrange(only_animation,brms_rt_both,rt)
...
d.f_50s <- arrange(only_animation,brms_rt_both,rt)
...
mean(all_summary$alpha)
sd(all_summary$alpha)
mean(all_summary$beta)
sd(all_summary$beta)

# regular ----
cor_matrix <- all_summary[,c(8,15,13,22,4,6,2,12,24:25)] # for presentation only
...
a<- partial.r(cor_matrix, x = c("NPS both", "cb_acc_20s","cb_acc_40s", "cb_acc_60s","CB RT", "alpha"), y = c("Control RT both"), use="pairwise",method="pearson")
cp <- corr.p(a, n = 50, adjust="holm",alpha=.05)
print(cp) 




temp <- cor_matrix[complete.cases(cor_matrix[ , c("NPS both","Control RT both", "cb_acc_20s","cb_acc_40s", "cb_acc_60s","CB RT", "alpha")]),]
b <- pcor(temp[,c("NPS both","Control RT both", "cb_acc_20s","cb_acc_40s", "cb_acc_60s","CB RT", "alpha")])
...
bh_table <- bh_table[rn != "NPS 1"]
bh_table <- bh_table[rn != "brms_rt"]
bh_table <- bh_table[rn != "Control RT both"]
...
cor.test(cor_matrix$`NPS both`, cor_matrix$`20 s accuracy`)
cor.test(all_summary$view_dist_mm, all_summary$brms_rt_1)


# without two worst deciles ----
cor_matrix <- all_summary[brms_quentile < 9,c(8,15,13,22,4,6,2,12,24:25)] # for presentation only
...
# control for control task
temp <- all_summary[complete.cases(all_summary[ , c('brms_rt_both', 'cb_acc_20s', 'control_rt_both')]),]
pcor.test(x=temp[,brms_rt_both], y=temp[, cb_acc_20s], z=temp[, control_rt_both]) # account for control

ggplot(all_summary, aes(x = brms_rt_both, y = cb_acc_20s)) +
  geom_point() +
  geom_smooth(method='lm') +
  ggtitle("Correlation between NPS and CB accuracy (20s)")+
  stat_cor(method="pearson", color = 'red', label.y.npc="top", label.x.npc = "center" )
ggplot(all_summary, aes(x = control_rt_both, y = cb_acc_20s)) +
  geom_point() +
  geom_smooth(method='lm') +
  ggtitle("Correlation between Control and CB accuracy (20s)")+
  stat_cor(method="pearson", color = 'red', label.y.npc="top", label.x.npc = "center" )


temp <- all_summary[complete.cases(all_summary[ , c('brms_rt', 'alpha', 'control_rt_1')]),]
pcor.test(x=temp[,brms_rt], y=temp[, alpha], z=temp[, control_rt_1]) # account for control

# Check only on participants with NPS of less then 5 sec ----
cor_matrix <- all_summary[brms_rt_both < 5000,c(2,4,6,8:9,12,14,19,23:24)]
...
ggplot(cor_matrix, aes(x = brms_rt_both, y = beta)) +
  geom_point() +
  geom_smooth(method='lm') +
...
cor_matrix <- all_summary[brms_rt_both < 10000,c(2,4,6,8:9,12,14,19,23:24)]
setnames(cor_matrix, 'cb_acc', "cb_acc_60s")
setnames(cor_matrix, 'cb_rt_with_unfinished', "cb_rt")
...
cor_matrix <- all_summary[brms_rt_both < 20000,c(2,4,6,8:9,12,14,19,23:24)]
setnames(cor_matrix, 'cb_acc', "cb_acc_60s")
setnames(cor_matrix, 'cb_rt_with_unfinished', "cb_rt")
cor_results <- corr.test(cor_matrix, adjust = "hoch")
...
cor.test(all_summary$brms_rt_both, all_summary$beta)
ggplot(all_summary, aes(x = brms_rt_both, y = beta)) +
  geom_point() +
  geom_smooth(method='lm')

cor.test(all_summary$cb_rt_with_unfinished, all_summary$view_dist_mm)
ggplot(all_summary, aes(x = cb_rt_with_unfinished, y = view_dist_mm)) +
  geom_point() +
  geom_smooth(method='lm')

cor.test(all_summary$cb_acc_40s, all_summary$view_dist_mm)
ggplot(all_summary, aes(x = cb_acc_40s, y = view_dist_mm)) +
  geom_point() +
  geom_smooth(method='lm')

ggplot(all_summary, aes(x = brms_rt_1, y = cb_acc_20s)) +
  geom_point() +
  geom_smooth(method='lm') +
  ggtitle("Correlation between NPS and alpha")+
  stat_cor(method="pearson", color = 'red', label.y.npc="top", label.x.npc = "center" )

ggplot(all_summary, aes(x = control_rt_1, y = cb_acc_20s)) +
  geom_point() +
  geom_smooth(method='lm') +
  ggtitle("Correlation between Control and alpha")+
  stat_cor(method="pearson", color = 'red', label.y.npc="top", label.x.npc = "center" )




# look only on participants with NPS of less then 20 sec
all_summary <- data.table(all_summary)
cor.test(all_summary[brms_rt_both < 20000, brms_rt_both], all_summary[brms_rt_both < 20000, alpha])
ggplot(all_summary[brms_rt_both < 20000], aes(x = brms_rt_both, y = alpha)) +
  geom_point() +
  geom_smooth(method='lm') 
pcor.test(x=all_summary[brms_rt_both < 20000, brms_rt_both], y=all_summary[brms_rt_both < 20000, alpha], z=all_summary[brms_rt_both < 20000, distance]) # account for distance

model <- lm(brms_rt_both ~ alpha + cb_acc_20s + view_dist_mm, data = all_summary)
summary(model)
...
pcor.test(x=all_summary$cb_rt_with_unfinished, y=all_summary$brms_rt_both, z=all_summary$view_dist_mm)
...
all_summary <- data.table(all_summary)
cor.test(all_summary[brms_rt_both > 20000]$brms_rt_both, all_summary[brms_rt_both > 20000]$cb_rt_with_unfinished)
ggplot(all_summary[brms_rt_both > 20000], aes(x = brms_rt_both, y = cb_rt_with_unfinished)) +
  geom_point() +
  geom_smooth(method='lm')

cor.test(all_summary[brms_quentile > 8 | brms_quentile < 3]$brms_rt_both, all_summary[brms_quentile > 8 | brms_quentile < 3]$cb_rt_with_unfinished)
ggplot(all_summary[brms_quentile > 8 | brms_quentile < 3], aes(x = brms_rt_both, y = cb_rt_with_unfinished)) +
  geom_point() +
  geom_smooth(method='lm')

cor.test(all_summary[brms_quentile > 8 | brms_quentile < 3]$brms_rt_both, all_summary[brms_quentile > 8 | brms_quentile < 3]$beta)
ggplot(all_summary[brms_quentile > 8 | brms_quentile < 3], aes(x = brms_rt_both, y = beta)) +
  geom_point() +
  geom_smooth(method='lm')
...
stimuli <- stimuli[stimulus_rank <= 30] # to remove unwanted empty stimuli
...
sd(temp[stimulus_rank > 20 & brms_group == 'best', rt])
sd(temp[stimulus_rank > 20 & brms_group == 'worst', rt])
## Classical example: Student's sleep data
...
#only_animation <- merge(only_animation, all_summary[, c("id","cb_rt_with_unfinished", "alpha", "beta", "cb_acc_20s", "cb_acc_40s", "total_hsp")], by="id") # disabled for now because total_hsp isnt working yet
only_animation <- merge(only_animation, all_summary[, c("id","cb_rt_with_unfinished", "alpha", "beta", "cb_acc_20s", "cb_acc_40s")], by="id")
...
# bRMS NPS
cor.test(easy_stimuli$brms_rt_both, easy_stimuli$rt)
p3 <- ggplot(easy_stimuli, aes(x = brms_rt_both, y = rt)) +
  geom_point() +
  geom_smooth(method='lm') +
  ggtitle("Easy stimuli") +
  stat_cor(method="pearson", color = 'red', label.y.npc="top", label.x.npc = "center" )

cor.test(hard_stimuli$brms_rt_both, hard_stimuli$rt)
p4 <- ggplot(hard_stimuli, aes(x = brms_rt_both, y = rt)) +
  geom_point() +
  geom_smooth(method='lm') +
  ggtitle("Hard stimuli") +
  stat_cor(method="pearson", color = 'red', label.y.npc="top", label.x.npc = "center" )
...
# Test 2 best and worst deciles ----
big_data <- data.table::rbindlist(datalist)
big_data <- merge(big_data, all_summary[,c("id", "brms_quentile")], by = "id")
big_data[brms_quentile > 8, rank := "worst"]
big_data[brms_quentile < 3, rank := "best"]
big_data[brms_quentile >= 3 & brms_quentile <= 8, rank := "medium"]
big_data <- big_data[rank != "medium"]
big_data <- data.frame(big_data)
big_data <- big_data[,c(2:3,5)]
big_data$time_sec <- as.factor(big_data$time_sec)
...
p<- ggplot(df.summary, aes(x=time_sec, y=mean, group=rank, color=rank)) + 
  ...
  geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.5,
                position=position_dodge(0.4))
p + labs(title="Accumulated CB proportion correct between 3 best and 3 worst NPS deciles", x="Time (sec)", y = "% correct")+
  theme_classic() +
  annotate("text", x=40, y=0.2, label= paste("d = ", round(as.numeric(ks$statistic), 2), "   p = ", round(as.numeric(ks$p.value), 5)))


# Test 3 best and worst deciles ----
big_data <- data.table::rbindlist(datalist)
big_data <- merge(big_data, all_summary[,c("id", "brms_quentile")], by = "id")
big_data[brms_quentile > 6, rank := "worst"]
big_data[brms_quentile < 4, rank := "best"]
big_data[brms_quentile >= 4 & brms_quentile <= 6, rank := "medium"]
big_data <- big_data[rank != "medium"]
big_data <- data.frame(big_data)
big_data <- big_data[,c(2:3,5)]
big_data$time_sec <- as.factor(big_data$time_sec)
...
# test 4 best and worse deciles
big_data <- data.table::rbindlist(datalist)
big_data <- merge(big_data, all_summary[,c("id", "brms_quentile")], by = "id")
big_data[brms_quentile > 5, rank := "worst"]
big_data[brms_quentile < 5, rank := "best"]
big_data[brms_quentile == 5, rank := "medium"]
big_data <- big_data[rank != "medium"]
big_data <- data.frame(big_data)
big_data <- big_data[,c(2:3,5)]
big_data$time_sec <- as.factor(big_data$time_sec)
...
p<- ggplot(df.summary, aes(x=time_sec, y=mean, group=rank, color=rank)) + 
  ...
  geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.5,
                position=position_dodge(0.4))
p + labs(title="Accumulated CB proportion correct between 4 best and 4 worst NPS deciles", x="Time (sec)", y = "% correct")+
  theme_classic() +
  annotate("text", x=40, y=0.2, label= paste("d = ", round(as.numeric(ks$statistic), 2), "   p = ", round(as.numeric(ks$p.value), 5)))


# test only 3 best and worst deciles
big_data <- data.table::rbindlist(datalist)
big_data <- merge(big_data, all_summary[,c("id", "brms_quentile")], by = "id")
big_data[brms_quentile > 7, rank := "worst"]
big_data[brms_quentile < 3, rank := "best"]
big_data[brms_quentile < 8 & brms_quentile > 2, rank := "medium"]
big_data <- big_data[rank != "medium"]
big_data <- data.frame(big_data)
big_data <- big_data[,c(2:3,5)]
big_data$time_sec <- as.factor(big_data$time_sec)
...
p<- ggplot(df.summary, aes(x=time_sec, y=mean, group=rank, color=rank)) + 
  ...
  geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd), width=.5,
                position=position_dodge(0.4))
p + labs(title="Accumulated CB proportion correct between the best and worst NPS deciles", x="Time (sec)", y = "% correct")+
  theme_classic() +
  annotate("text", x=40, y=0.2, label= paste("d = ", round(as.numeric(ks$statistic), 2), "   p = ", round(as.numeric(ks$p.value), 7)))



# Dividing by reported driving ability 
only_cocky_drivers <- all_summary[driving_ability < 4]
only_cocky_drivers <- only_cocky_drivers[driving_accidents < 10]

cor.test(only_cocky_drivers$brms_rt_both, only_cocky_drivers$driving_ability)
ggplot(only_cocky_drivers, aes(x = brms_rt_both, y = driving_ability)) +
  geom_point() +
  geom_smooth(method='lm') +
  ggtitle("Correlation between NPS and driving ability")+
  stat_cor(method="pearson", color = 'red', label.y.npc="top", label.x.npc = "center" )

cor.test(only_cocky_drivers$brms_rt_both, only_cocky_drivers$driving_accidents)
ggplot(only_cocky_drivers, aes(x = brms_rt_both, y = driving_accidents)) +
  geom_point() +
  geom_smooth(method='lm') +
  ggtitle("Correlation between NPS and driving accidents")+
  stat_cor(method="pearson", color = 'red', label.y.npc="top", label.x.npc = "center" )
pcor.test(x=only_cocky_drivers$brms_rt_both, y=only_cocky_drivers$driving_accidents, z=only_cocky_drivers$age) # account for distance


# Look at CB stimuli rt in comparison with stimuli Database ----
cb_stimuli <- aggregate(only_animation[,rt], by = list(only_animation$stimulus), FUN = mean)
setnames(cb_stimuli, 1:2, c("stimulus", "mean_rt"))
cb_db <- read.csv('cb_db.csv')
cb_db <- na.omit(cb_db)
cb_db$stimulus <- sub('"', '', cb_db$stimulus)
cb_stimuli <- merge(cb_stimuli, cb_db)
cor.test(only_cocky_drivers$brms_rt_both, only_cocky_drivers$driving_ability)
ggplot(cb_stimuli, aes(x = mean_rt/1000, y = db_rt)) +
  geom_point() +
  geom_smooth(method='lm') +
  ggtitle("Correlation between experiment rt and database rt (CB stimuli)")+
  stat_cor(method="pearson", color = 'red', label.y.npc="bottom", label.x.npc = "center" ) +
  xlab("Experiment mean rt (sec)") +
  ylab("Database mean rt (sec)") +
  geom_abline(slope=1, intercept = 0, color = "red", linetype = "dashed")


cor.test(only_cocky_drivers$brms_rt_both, only_cocky_drivers$driving_ability)
ggplot(all_summary[brms_rt_both <5000,], aes(x = brms_rt_both, y = beta)) +
  geom_point() +
  geom_smooth(method='lm') +
  ggtitle("")+
  stat_cor(method="pearson", color = 'red', label.y.npc="top", label.x.npc = "center" )
...
ggplot(all_summary, aes(x = brms_rt_1, y = brms_rt_2)) +
...
ggplot(all_summary, aes(x = control_rt_1, y = control_rt_2)) +
  geom_point(aes(color = time_diff), size = 3) +
  scale_color_viridis_c()  +
  geom_smooth(method='lm') +
  stat_cor(method="pearson", color = 'red', label.y.npc="top", label.x.npc = "center" )

ggplot(all_summary, aes(x = brms_rt_1, y = control_rt_1)) +
  geom_point() +
  geom_smooth(method='lm') +
  stat_cor(method="pearson", color = 'red', label.y.npc="top", label.x.npc = "left" )
...
# dems histograms ----
all_summary$total_hsp <- all_summary$total_hsp+1
all_summary$driving_ability <- all_summary$driving_ability+1


# HSP
h1 = hist(all_summary$total_hsp, breaks = c(seq(from = 1, to = 7, by = 1)), plot = F) # or hist(x,plot=FALSE) to avoid the plot of the histogram
h1$density = h1$counts/sum(h1$counts)*100

# Age
h2 = hist(all_summary$age, breaks = c(seq(from = 10, to = 80, by = 5)), plot = F) # or hist(x,plot=FALSE) to avoid the plot of the histogram
...
# Driver ability
h4 = hist(all_summary$driving_ability, breaks = c(seq(from = 1, to = 7, by = 1)), plot = F) # or hist(x,plot=FALSE) to avoid the plot of the histogram
...
cor_matrix <- all_summary[,c('age', 'total_hsp','driving_accidents','driving_ability','pedestrian_accidents', 'brms_rt_both', 'cb_rt_with_unfinished', 'cb_acc_20s', 'cb_acc_40s', 'alpha', 'beta')]
cor_results <- corr.test(cor_matrix, adjust = "hoch")
corrplot(cor_results$r, p.mat = cor_results$p, method = "color", tl.col = "black", addCoef.col = "black", insig = "blank")

cor.test(all_summary$brms_rt_both, all_summary$total_hsp)
ggplot(all_summary, aes(x = brms_rt_both, y = total_hsp)) +
  geom_point() +
  geom_smooth(method='lm') +
  ggtitle("Correlation between NPS and HSP")+
  stat_cor(method="pearson", color = 'red', label.y.npc="top", label.x.npc = "center" )

cor.test(all_summary$brms_rt_both, all_summary$driving_ability)
ggplot(all_summary, aes(x = brms_rt_both, y = driving_ability)) +
  geom_point() +
  geom_smooth(method='lm') +
  ggtitle("Correlation between NPS and driving ability")+
  stat_cor(method="pearson", color = 'red', label.y.npc="top", label.x.npc = "center" )

cor.test(all_summary$brms_rt_both, all_summary$total_accidents)
ggplot(all_summary, aes(x = brms_rt_both, y = total_accidents)) +
  geom_point() +
  geom_smooth(method='lm') +
  ggtitle("Correlation between NPS and driving accidents")+
  stat_cor(method="pearson", color = 'red', label.y.npc="top", label.x.npc = "center" )

cor_matrix <- all_summary[,c( 'brms_rt_both', 'total_hsp','driving_ability','total_accidents')]

setnames(cor_matrix, 'total_hsp', "HSP score")
setnames(cor_matrix, 'total_accidents', "Number of accidents")
...
temp <- all_summary[complete.cases(all_summary[ , c('brms_rt_both', 'cb_rt', 'age')]),]
pcor.test(x=temp[,brms_rt_both], y=temp[, cb_rt_with_unfinished], z=temp[, age]) # account for control
cor.test(all_summary$brms_rt_both, all_summary$cb_rt_with_unfinished)

# check dems with Experiment 1 + 2 ----

# load exp 1 dems data
setwd ("C:/Users/yuval/Downloads")
dems1 <- fread('pilot_dems.csv')
dems1[,pedestrian_accidents := as.numeric(pedestrian_accidents)] # make numeric
dems1[,driving_accidents := as.numeric(driving_accidents)] # make numeric
dems1$total_accidents <- rowSums(dems1[, c('driving_accidents', 'pedestrian_accidents')], na.rm = T)
dems1 <- dems1[, c('id', 'total_accidents', 'total_hsp', 'brms_rt')]


dems2 <- all_summary[, c('id', 'total_accidents', 'total_hsp', 'brms_rt_both')]
colnames(dems2)[4] <- 'brms_rt'

dems_combined <- rbind(dems1, dems2)

cor_matrix <- dems_combined[, c('total_accidents', 'total_hsp', 'brms_rt')]
...
ggplot(all_summary, aes(x = control_rt_both, y = cb_acc)) +
  geom_point() +
  geom_smooth(method='lm') +
  stat_cor(method="pearson", color = 'red', label.y.npc="bottom", label.x.npc = "center" )

ggplot(all_summary, aes(x = control_rt_both, y = alpha)) +
  geom_point() +
  geom_smooth(method='lm') +
  stat_cor(method="pearson", color = 'red', label.y.npc="bottom", label.x.npc = "center" )


write.csv(all_summary,"C:/Users/yuval/Downloads/all_summary.csv", row.names = FALSE)
