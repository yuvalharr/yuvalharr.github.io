...
h = hist(cb_summary$cb_rt/1000, breaks = c(seq(from = 0, to = 60, by = 2.5)), plot = F) # or hist(x,plot=FALSE) to avoid the plot of the histogram
h$density = h$counts/sum(h$counts)*100
plot(h,freq=FALSE)
...
for (i in unique(d.f$id)) {
  ...
  fit <- nls(cum_correct ~ alpha*exp(b/rt_sec),data=participant_1,
             algorithm="port",
             start=c(alpha=1,b=-2),lower=c(alpha=0,b=-Inf), upper=c(alpha=1.5,b=0))
  summary(fit)
...
hard_stimuli <-  only_animation[stimulus_rank <= 10]
